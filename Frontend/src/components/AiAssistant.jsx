...
export default function AiAssistant() {
  ...
  const send = async (e) => {
    ...
    try {
      ...
      setErr(e.response?.data?.message || "Couldn't reach the assistant. Please try again.");
    } finally {
      setLoading(false);
    }
  };
        ...
        <div className="ai-panel panel">
          ...
          <form className="ai-input-row" onSubmit={send}>
            <input
              ...
              disabled={loading}
            />
            <button type="submit" className="primary-btn" disabled={loading || !input.trim()}>Send</button>
          </form>
        </div>
      )}
      <button
