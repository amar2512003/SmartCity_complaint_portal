...
export default function CitizenLayout(){
 ...
 return <div className="shell" style={{display:'flex',flexDirection:'column'}}><header className="app-nav"><div className="nav-inner"><Link className="brand" to="/dashboard"><img src={brandMark} alt="SmartCity" className="brand-mark" /><span>Samadhan.</span></Link><nav className="nav-links"><Link className={location.pathname==='/dashboard'?'active':''} to="/dashboard">{t('nav.dashboard')}</Link><Link className={location.pathname==='/grievance/new'?'active':''} to="/grievance/new">{t('nav.reportIssue')}</Link></nav><div className="nav-user"><div className="avatar">{initials}</div><span className="user-name">{user?.name}</span><button className="ghost-btn mobile-hide" onClick={logout}>{t('nav.signOut')}</button></div></div></header><div className="lang-toggle-bar"><div className="lang-toggle-bar-inner"><LanguageToggle /></div></div><main className="container page" style={{flex:1}}><Outlet/></main><Footer/><AiAssistant/><InstructionsGuide/></div>
}
